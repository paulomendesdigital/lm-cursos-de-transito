...
<div id="plugin_acl">
	...
	if(!isset($no_acl_links))
	{
        ...
        $links = array();
        $links[] = $this->Html->link(__d('acl', 'Permissions'), '/manager/acl/aros/index', array('class' => ($selected == 'aros' )? 'selected' : null));
        $links[] = $this->Html->link(__d('acl', 'Actions'), '/manager/acl/acos/index', array('class' => ($selected == 'acos' )? 'selected' : null));
