...
class AclAppController extends AppController
{
	...
	private function _check_config()
	{
		...
		if(!empty($role_model_name))
		{
	    	...
	    	$this->_authorize_managers();
	    	
//	    	if($this->name != 'Acl'
//	    		&&
//	    	   ($this->name != 'Acos' || $this->action != 'manager_build_acl')
//	    	  )
//	    	{
...
//	    		    $this->render('/Acos/manager_acos_missing');
//	    		}
//	    	}
	    	...
	    	if(Configure :: read('acl.check_act_as_requester'))
	    	{
	    		...
	    		if(!$is_requester)
	    		{
	    			$this->render('/Aros/manager_not_acl_requester');
	    		}
	    	}
		}
		else
	...
	function _check_files_updates()
	{
	    if($this->request->params['controller'] != 'acos'
	        || ($this->request->params['action'] != 'manager_synchronize' &&
	            $this->request->params['action'] != 'manager_prune_acos' &&
	            $this->request->params['action'] != 'manager_build_acl'))
	    {
    	    if($this->AclManager->controller_hash_file_is_out_of_sync())
    	    {
        		...
        		if($has_updates)
        		{
        		    $this->render('/Acos/manager_has_updates');
        		    $this->response->send();
	...
	private function _authorize_managers()
	{
	...
	{
	    $this->redirect($this->referer(array('action' => 'manager_index')));
	}
}
?>