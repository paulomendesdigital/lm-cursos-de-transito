...
class ArosController extends AclAppController
{
	...
	function manager_check($run = null)
	{
	...
	function manager_update_user_role()
	{
	    $user_model_name = Configure :: read('acl.aro.user.model');
	...
	function manager_ajax_role_permissions()
	{
		$role_model_name = Configure :: read('acl.aro.role.model');
	...
	function manager_role_permissions()
	{
	    $role_model_name = Configure :: read('acl.aro.role.model');
	...
	function manager_user_permissions($user_id = null)
	{
	    $user_model_name = Configure :: read('acl.aro.user.model');
	    $role_model_name = Configure :: read('acl.aro.role.model');
	    ...
	    else
	    {
            ...
            if(isset($this->params['named']['ajax']))
            {
                $this->render('manager_ajax_user_permissions');
            }
	    }
	}

	function manager_empty_permissions()
	{
	...
	function manager_clear_user_specific_permissions($user_id)
	{
	...
	function manager_grant_all_controllers($role_id)
	{
	...
	function manager_deny_all_controllers($role_id)
	{
	    $role =& $this->{Configure :: read('acl.aro.role.model')};
        $role->id = $role_id;
	...
	function manager_get_role_controller_permission($role_id)
	{
		$role =& $this->{Configure :: read('acl.aro.role.model')};
	...
	function manager_grant_role_permission($role_id)
	{
	    $role =& $this->{Configure :: read('acl.aro.role.model')};
	...
	function manager_deny_role_permission($role_id)
	{
	    $role =& $this->{Configure :: read('acl.aro.role.model')};
	...
	function manager_get_user_controller_permission($user_id)
	{
        $user =& $this->{Configure :: read('acl.aro.user.model')};
	...
	function manager_grant_user_permission($user_id)
	{
	    $user =& $this->{Configure :: read('acl.aro.user.model')};
	...
	function manager_deny_user_permission($user_id)
	{
	    $user =& $this->{Configure :: read('acl.aro.user.model')};
