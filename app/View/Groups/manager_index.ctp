...
<div class="table-responsive">
	...
	<tr>
		...
		<td><?php echo $this->Utility->__LimitText($group['Group']['name']); ?></td>
		<td><?php echo $this->Utility->__FormatDate($group['Group']['created']); ?></td>
		<td><?php echo $this->Utility->__FormatDate($group['Group']['modified']); ?></td>
		<td class="actions">
				<?php echo $this->Html->link(__('<span class="icon-eye"></span>'), array('action' => 'view', $group['Group']['id']), array('class' => 'btn btn-info btn-xs', 'escape' => false)); ?>
			<?php echo $this->Html->link(__('<span class="icon-pencil"></span>'), array('action' => 'edit', $group['Group']['id']), array('class' => 'btn btn-primary btn-xs', 'escape' => false)); ?>
			<?php echo $this->Form->postLink(__('<span class="icon-remove3"></span>'), array('action' => 'delete', $group['Group']['id']), array('class' => 'btn btn-danger btn-xs', 'escape' => false), __('Are you sure you want to delete # %s?', $group['Group']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
        </tbody>
