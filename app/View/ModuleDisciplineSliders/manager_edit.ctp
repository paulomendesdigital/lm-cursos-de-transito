...
<?php echo $this->Form->create('ModuleDisciplineSlider', array('class'=>'form-horizontal','role'=>'form','type' => 'file')); ?>
    ...
    <div class="tabbable">
        ...
        <div class="tab-content with-padding">
            ...
            <?php
                ...
                echo $this->Form->input('audio', array('type' => 'file','class' => 'form-control', 'label'=>__('audio')));
                ?>
                <?php if($this->data['ModuleDisciplineSlider']['audio']):?>
                    <div class='form-group'>
                        <label class='col-sm-2 control-label text-right'>
                            <?php echo __('remover'); ?> audio
                        </label>
                        <div class='col-sm-10'>
                            <span class='text-primary'>
                                <?php echo $this->data['ModuleDisciplineSlider']['audio'];?>
                            </span>
                            <div class='checkbox'>
                                <label>
                                    <div>
                                        <span><input type='checkbox' class='styled' name='data[ModuleDisciplineSlider][audio][remove]'></span>
                                    </div>
                ...
                <div class='form-group'>
                    ...
                    <div class='col-sm-10'>
                        <a class='btn <?php echo isset($this->request->data['ModuleDisciplineSlider']['status']) ? $this->request->data['ModuleDisciplineSlider']['status'] == 0 ? 'btn-danger' : '' : 'btn-danger'; ?> btn-sm' data-id='0' data-toggle='btnStatus'>Inativo</a>
                        <?php echo $this->Form->input('status', array('type' => 'hidden', 'data-toggle' => 'afferStatus'));?>
                        <a class='btn <?php echo isset($this->request->data['ModuleDisciplineSlider']['status']) ? $this->request->data['ModuleDisciplineSlider']['status'] == 1 ? 'btn-success' : '' : ''; ?> btn-sm' data-id='1' data-toggle='btnStatus'>Ativo</a>
                    </div>
                </div>
                <?php echo $this->Form->input('position', array('class' => 'form-control', 'data-mask' => 'position', 'label'=>__('position')));?>
            </div>
        </div>
    </div>
</div><!-- /block-->
