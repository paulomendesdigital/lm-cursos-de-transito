...
<div class="row">
    ...
    <div class="col-sm-12 col-md-2">
        <div class="form-group">
        <?php echo $this->Search->input('filterCode', array('class' => 'form-control', 'placeholder'=>'Código do cupom'));?> 
        </div>
    </div>
    <div class="col-sm-12 col-md-2">
        <div class="form-group">
        ...
        <tbody>
            <?php foreach ($tickets as $ticket): ?>
            	<tr>
                    ...
                    <td><?php echo h($ticket['Ticket']['code']); ?></td>
            		<td><?php echo h($ticket['Ticket']['value']) . ' %'; ?></td>
            		<td><?php echo $this->Utility->__FormatStatus($ticket['Ticket']['status']); ?></td>
            		<td><?php echo $ticket['User']['name']; ?>
            		</td>
            		<td><?php echo $this->Utility->__FormatDate($ticket['Ticket']['created']); ?></td>
            		<td><?php echo $this->Utility->__FormatDate($ticket['Ticket']['modified']); ?></td>
            		<td class="actions">
            			<?php echo $this->Html->link(__('<span class="icon-eye"></span>'), array('action' => 'view', $ticket['Ticket']['id']), array('class' => 'btn btn-info btn-xs', 'escape' => false)); ?>
            			<?php echo $this->Html->link(__('<span class="icon-pencil"></span>'), array('action' => 'edit', $ticket['Ticket']['id']), array('class' => 'btn btn-primary btn-xs', 'escape' => false)); ?>
            			<?php echo $this->Form->postLink(__('<span class="icon-remove3"></span>'), array('action' => 'delete', $ticket['Ticket']['id']), array('class' => 'btn btn-danger btn-xs', 'escape' => false), __('Are you sure you want to delete # %s?', $ticket['Ticket']['id'])); ?>
            		</td>
            	</tr>
            <?php endforeach; ?>
        </tbody>
