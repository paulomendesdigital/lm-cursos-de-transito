...
<div class="row">
    ...
    <div class="col-sm-12 col-md-2">
        <div class="form-group">
        <?php echo $this->Search->input('filterStatus',array('class'=>'form-control'));?> 
        </div>
    </div>
    <div class="col-sm-12 col-md-2">
        ...
        <tbody>
            <?php foreach ($instructors as $instructor): ?>
            	<tr>
            		...
            		<td><?php echo $instructor['User']['name']; ?></td>
            		<td><?php echo $this->Utility->__LimitText($instructor['Instructor']['name']); ?></td>
            		<td><?php echo $this->Utility->__FormatStatus($instructor['Instructor']['status']); ?></td>
            		<td><?php echo $this->Utility->__FormatDate($instructor['Instructor']['created']); ?></td>
            		<td><?php echo $this->Utility->__FormatDate($instructor['Instructor']['modified']); ?></td>
            		<td class="actions">
        				<?php echo $this->Html->link(__('<span class="icon-eye"></span>'), array('action' => 'view', $instructor['Instructor']['id']), array('class' => 'btn btn-info btn-xs', 'escape' => false)); ?>
            			<?php echo $this->Html->link(__('<span class="icon-pencil"></span>'), array('action' => 'edit', $instructor['Instructor']['id']), array('class' => 'btn btn-primary btn-xs', 'escape' => false)); ?>
            			<?php echo $this->Form->postLink(__('<span class="icon-remove3"></span>'), array('action' => 'delete', $instructor['Instructor']['id']), array('class' => 'btn btn-danger btn-xs', 'escape' => false), __('Are you sure you want to delete # %s?', $instructor['Instructor']['id'])); ?>
            		</td>
            	</tr>
            <?php endforeach; ?>
        </tbody>
